...
def getTokens(p, AFD, sf, tokens):
    temp = ''
    for i in AFD:
        if '*' in i:
            for j in range(0, len(i[2])):
                if '<'+i[2][j]+'>' == p and '<'+i[2][j]+'>' not in tokens:
                    temp += getTokens(i[1] ,AFD, sf, tokens)
                    temp += sf[j]
                    return temp
        else:
            for j in range(0, len(i[1])):
                if '<'+i[1][j]+'>' == p and '<'+i[1][j]+'>' not in tokens:
                    temp += getTokens(i[0] ,AFD, sf, tokens)
                    temp += sf[j]
                    return temp
    return temp

def print_tokens(AFD, A):
    nome_arquivo = 'tokens.in'
    arquivo = open(nome_arquivo, 'w+')
    sf = sorted(get_sf(A))
    tokens = {}
    for i in AFD:
